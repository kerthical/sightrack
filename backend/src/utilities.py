...
def find_max_confidence_box_and_score(boxes: np.ndarray, scores: np.ndarray) -> Tuple[Optional[np.array], Optional[float]]:
...
def smooth_value(current_value: float, previous_value: float, smoothing_factor: float = 0.3) -> float:
    return smoothing_factor * current_value + (1 - smoothing_factor) * previous_value


def estimate_gaze_point(image: np.array, yaw: float, pitch: float, head_box: Tuple[float, float, float, float]) -> Tuple[int, int]:
    return 0, 0
...
def apply_heatmap(image: np.array, gaze_x: int, gaze_y: int, radius: int) -> np.array:
    ...
    overlay = cv2.addWeighted(cv2.cvtColor(image, cv2.COLOR_BGR2BGRA), 1.0, colored_heatmap, 0.5, 0)
