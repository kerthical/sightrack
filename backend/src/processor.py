...
from utilities import (
    find_max_confidence_box_and_score,
    estimate_gaze_point,
    smooth_value,
    apply_heatmap,
)
...
class ImageProcessor:
    ...
    def process_frame(self, image: np.array) -> ImageProcessorResult:
        ...
        box, score = (
            find_max_confidence_box_and_score(boxes, scores) if boxes else (None, None)
        )
        yaw, pitch, roll, gaze_x, gaze_y = (
            self.resnet_model.infer(image, box, score) if box else (0, 0, 0, 0, 0)
        )
        ...
        if box:
            ...
            image = apply_heatmap(
                image, int(gaze_x), int(gaze_y), int((1 - score) * (box[2] - box[0]))
            )
